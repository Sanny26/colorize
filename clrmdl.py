...
def rgb2uv(img):
    rgb_to_yuv_kernel = [[0.299, -0.14714119,
...
if __name__ == "__main__":
    ...
    model.compile(loss = blur_uv_loss,
                    optimizer = 'adam',
                    metrics=['accuracy'])
    model.fit_generator(batch_generator(X, y, 3), steps_per_epoch=10, epochs= 50 ) 

    '''test_path = "test/"
    files = os.listdir(test_path)
    timages = np.zeros((len(files), 224, 224, 1))
    for i, f in enumerate(files):
        img = image.load_img(test_path+f, grayscale=True, target_size=(224, 224))
        timages[i] = image.img_to_array(img)

    pimages = model.predict(timages)    
    for i in range(len(files)):
        plt.imshow(pimages[i])
        plt.show()'''
